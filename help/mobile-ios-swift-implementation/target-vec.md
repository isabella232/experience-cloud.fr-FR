...
description: Découvrez comment mettre en œuvre le compositeur d’expérience visuelle (VEC) de Target avec des paramètres personnalisés, l’associer à votre appareil et créer une activité. Cette leçon fait partie du tutoriel Mise en œuvre d’Experience Cloud dans les applications mobiles Swift pour iOS.
...
* avoir suivi les leçons des sections [Configuration de Launch](launch-create-a-property.md) ;
...
1. Dans Xcode, ouvrez le fichier `BookingViewController.swift`. Ce fichier est utilisé par l’écran d’accueil.
...
1. Notez que l’**[!UICONTROL schéma d’URL]** est `BusBookingSwift`. Vous pouvez utiliser ce schéma ou le modifier si vous le souhaitez.
1. Assurez-vous que **[!UICONTROL Rôle]** est défini sur **[!UICONTROL Éditeur]**.

   ![Enregistrement du schéma d’URL](images/ios/swift/mobile-targetvec-registerScheme.png)
...
Désormais, lorsqu’un utilisateur qui a installé votre application ouvre une URL du type `BusBookingSwift://com.adobetarget.BusBookingSwift` (ou tout schéma que vous avez défini) dans le simulateur, votre application s’ouvrira.
...
1. Entrez l’URL `BusBookingSwift://com.adobetarget.BusBookingSwift` (ou le schéma que vous avez défini) dans la barre d’adresse. Si vous rencontrez des difficultés, consultez la section Conseil ci-dessous.
1. Une fenêtre modale devrait vous inviter à « Ouvrir cette page dans BusBookingSwift ». Si vous rencontrez des difficultés, consultez la section Conseil ci-dessous.
1. Cliquez sur `Open`.
1. L’application Bus Booking devrait s’ouvrir.
   ...
   ![Vérification du lien profond](images/ios/swift/mobile-targetvec-verifyDeepLink.png)
...
1. Saisissez le schéma d’URL que vous venez de définir dans le champ **[!UICONTROL Saisir le schéma d’URL]**, par ex. `BusBookingSwift://com.adobetarget.BusBookingSwift`.
1. Cliquez sur **[!UICONTROL Créer un lien profond]**.

   ![Saisie de votre schéma d’URL et création du lien profond](images/ios/swift/mobile-targetvec-enterURLScheme.png)
...
1. Cliquez n’importe où sur le lien pour le copier automatiquement dans le presse-papiers.
   ...
   ![Copie de l’URL](images/ios/swift/mobile-targetvec-copyURL.png)
...
1. Cliquez sur le bouton `Open` pour ouvrir l’application.
   ...
   ![Collage de l’URL dans Safari](images/ios/swift/mobile-targetvec-pasteURL.png)
...
1. Cliquez sur des ressources de texte et d’image dans votre application : des options permettant de les modifier et de les remplacer devraient s’afficher.

   ![Chargement de l’application dans le VEC](images/ios/swift/mobile-targetvec-devicePaired.png)
...
1. Au sein d’une même activité, vous pouvez mettre à jour plusieurs affichages dans votre application
   ![Chargement de l’application dans le VEC](images/ios/swift/mobile-targetvec-navigateTheApp.png)
...
![Utilisation d’un appareil enregistré](images/ios/swift/mobile-targetvec-useSavedApp.png)
